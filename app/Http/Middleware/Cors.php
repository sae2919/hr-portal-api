...
class Cors
{
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($request->getMethod() === 'OPTIONS') {
            return response('', 200)
                ->header('Access-Control-Allow-Origin', $request->header('Origin', 'http://127.0.0.1:3000'))
        ...
        $response->headers->set('Access-Control-Allow-Origin', $request->header('Origin', 'http://127.0.0.1:3000'));
